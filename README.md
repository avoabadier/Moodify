...
Reads the room — weather + time of day where you are — and hands you one Spotify song for right now. No login, no playlists, no library to manage: open the app, see a mood, get a song, tap play.
...
- The song-picking pipeline is decent, not bulletproof — occasional off-mood, generic-filler, or non-Filipino picks still slip through despite several rounds of tuning. See [BACKEND_REPORT.md](BACKEND_REPORT.md) for the honest version of how well it actually works.
...
- [spec.md](spec.md) — the product/technical spec (what it does, and why it's built the way it is)
- [GUIDE.md](GUIDE.md) — a beginner-oriented walkthrough of how it's built and how to tweak it, with a focus on the Spotify API side
- [BACKEND_REPORT.md](BACKEND_REPORT.md) — a deep dive on the backend and specifically how track search/selection works, including its real limitations
- [PROJECT_REPORT.md](PROJECT_REPORT.md) — a status snapshot: what's built, what's changed, what's still open
